...
function crearClaseProfesor() {
  ...
  class Profesor {
    ...
    getCursos() {
      ...
      for (let prop in this.cursos) {
        arreTodos.push(this.cursos[prop]);
      }
    ...
    getUltimoCertificado() {
      ...
      var newObjeto ={};
        ...
        for(var i = 0; i < this.certificados.length; i++){
          for(var j = 0; j< this.certificados.length - 1; j++){

            if(this.certificados[i].anio > this.certificados[j].anio){
              newObjeto = this.certificados[i];
            }
          }
        }
        return newObjeto;
